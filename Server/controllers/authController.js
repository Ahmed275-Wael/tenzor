...
exports.register = catchAsync(async (req, res, next) => {
  ...
  if (!data.email || !data.password)
    return next(new AppError("Please Enter an email and password"), 404);
...
exports.logIn = catchAsync(async (req, res, next) => {
  ...
  if (!user || !authTools.verifyPassword(data.password, user.password))
    return next(new AppError("Please Enter a correct email and password", 404));
  const token = authTools.generateToken(user);
  res.cookie("jwt", token, { maxAge: 900000, httpOnly: true });
  res.status(202).json({
    status: "Success",
...
exports.protectRoute = catchAsync(async (req, res, next) => {
  ...
  if (!user)
    return next(
      new AppError("Please sign in again error token is invalid", 404)
    );
  if (user.passwordUpdated) {
    if (tokenVeri.iat * 1000 < user.passwordUpdatedAt)
      return next(
        new AppError("Please Sign In Again as your pw changed recently"),
        404
      );
  }
  req.user = user;
...
exports.restrictedTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!req.user)
      return next(
        new AppError(
          "Please Login To Join This Route As it has restrictions",
          404
        )
      );
    if (!roles.includes(role))
      return next(
        new AppError("You are not authorized to enter that route", 404)
      );
    next();
  };
};
